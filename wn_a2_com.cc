...
main (int argc, char *argv[])
{
  ...
  // Address sinkAddress3 (InetSocketAddress (staInterface.GetAddress (0), sinkPort3));
  // PacketSinkHelper packetSinkHelper3 ("ns3::TcpSocketFactory", InetSocketAddress (Ipv4Address::GetAny (), sinkPort3));
  // ApplicationContainer sinkApps3 = packetSinkHelper3.Install (wifiStaNodes.Get (0));
  // sinkApps2.Start (Seconds (0.));
  // sinkApps2.Stop (Seconds (simulation_time));
  ...
  //  Ptr<Socket> ns3TcpSocket3 = Socket::CreateSocket (wifiStaNodes.Get(1), TcpSocketFactory::GetTypeId ());
  // Ptr<MyApp> app3 = CreateObject<MyApp> ();
  // app3->Setup (ns3TcpSocket3, sinkAddress3, 1460, 1000000, DataRate ("100Mbps"));
  // wifiStaNodes.Get(1)->AddApplication(app3);
  ...
  if (tracing)
    {
      ...
      // pointToPoint3.EnablePcapAll ("n0n3");
